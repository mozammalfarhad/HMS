...
{
    public partial class CultureSensitivityMedicine : BaseGeneralSetup
    {
        int CurrentPage = 0;
        int TotalRec = 0;
        int PageSize = 30;
        ...
        protected void BindData()
        {
            try
            {
                dtg = new bllCultureSensitivityMedicine().GetList(CurrentPage * PageSize, PageSize, "", false);
                if (dtg.Rows.Count > 0)
                ...
                foreach (DataRow dr in dtg.Rows)
                {
                    ...
                    dgvMain.Rows[RowNum].Tag = dr["MedicineId"];
                }
        ...
        void SaveData()
        {
            ...
            try
            {
                ...
                int success = new bllCultureSensitivityMedicine().Insert(new Model.CultureSensitivityMedicine(0, tbxMadicineName.Text, tbxDescription.Text));
                if (success > 0)
                {
        ...
        {
            dgvMain.Columns["MedicineName"].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
            dgvMain.Columns["MedicineName"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
        ...
        void UpdateData()
        {
            try
            {
               int success = new bllCultureSensitivityMedicine().Update(new Model.CultureSensitivityMedicine(_ID, tbxMadicineName.Text,tbxDescription.Text));
                if (success > 0)
                {
        ...
        {
            int success = new bllCultureSensitivityMedicine().Delete(_ID);
            if (success > 0)
            {
        ...
        private void dgvMain_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            tbxMadicineName.Text = dgvMain.SelectedRows[0].Cells[1].Value.ToString();
